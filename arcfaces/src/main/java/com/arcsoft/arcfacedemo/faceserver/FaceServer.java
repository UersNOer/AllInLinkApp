    ...
    private void initFaceList(Context context) {
        synchronized (this) {
            if (ROOT_PATH == null) {
                ...
                ROOT_PATH = Environment.getExternalStorageDirectory()+"/人脸识别";
...
//                    fis.read(feature);
//
//                    Log.d("人脸提取:", Arrays.toString(feature));
//                    fis.close();
//
...
//            }
    ...
    public void getFaceByteformDb(Context context,String userName) {

        List<UserFace> userFaces = FaceUtils.getFace(context,userName);
        if (userFaces.size()>0){
            if (  faceRegisterInfoList ==null){
                faceRegisterInfoList = new ArrayList<>();
            }
            faceRegisterInfoList.add(new FaceRegisterInfo(userFaces.get(0).faceBytes,userName));
        }
     ...
     * @return 是否注册成功
     */
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name, String loginName, String pwd) {
        synchronized (this) {
            ...
            try {
                ...
                if (code == ErrorInfo.MOK) {
                    ...
                    Log.d("人脸存入:",Arrays.toString(faceFeature.getFeatureData()));
                    ...
                    Log.d("kais","dd");
                    return  saveDb(context,nv21,width,height,faceInfo,loginName,pwd);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private boolean saveDb(Context context,byte[] nv21,int width, int height, FaceInfo faceInfo,String loginName,String pwd) {
        if (faceInfo == null || loginName==null || pwd == null){
            return false;
        }
        FaceFeature faceFeature = new FaceFeature();
        //特征提取
        ...
        UserFace userFace = new UserFace(loginName,pwd,faceFeature.getFeatureData());
        return FaceUtils.writeFace(context,userFace);

//        if ( width % 4 != 0 || nv21.length != width * height * 3 / 2){
//
//
//        }
//        return false;
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        for (int i = 0; i < faceRegisterInfoList.size(); i++) {
            ...
            Log.d("人脸对比当前:", Arrays.toString(faceFeature.getFeatureData()));
            Log.d("人脸对比注册过:", Arrays.toString(tempFaceFeature.getFeatureData()));
            faceEngine.compareFaceFeature(faceFeature, tempFaceFeature, faceSimilar);
            if (faceSimilar.getScore() > maxSimilar) {
                maxSimilar = faceSimilar.getScore();
                maxSimilarIndex = i;
               // Log.d("人脸对比通过:",faceSimilar.getScore()+"");
            }
        }
        isProcessing = false;
