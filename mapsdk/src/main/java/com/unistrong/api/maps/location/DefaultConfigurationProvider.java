...
public class DefaultConfigurationProvider implements IConfigurationProvider {
    ...
    protected SimpleDateFormat httpHeaderDateTimeFormat = new SimpleDateFormat(OSMTileConstants.HTTP_EXPIRES_HEADER_FORMAT, Locale.US);
    ...
    public File getOsmdroidBasePath() {
        if (osmdroidBasePath==null)
            osmdroidBasePath = new File(StorageUtils.getStorage().getAbsolutePath(), "unismap");
        try {
    ...
    public void load(Context ctx, SharedPreferences prefs) {
        ...
        if (Build.VERSION.SDK_INT >= 9) {
            ...
            long cacheSize = 0;
            File dbFile = new File(getOsmdroidTileCache().getAbsolutePath() + File.separator + 0x00000080);
            if (dbFile.exists()) {
